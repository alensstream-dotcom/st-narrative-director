...
(async()=>{
 ...
 if(process.argv.includes('--models')){
   ...
   await page.getByText(/已获取 \d+ 个模型/).waitFor({timeout:30000});
   await page.getByRole('combobox',{name:'可用模型'}).selectOption('gpt-6-sol');
   if(await page.getByRole('textbox',{name:'模型名称'}).inputValue()!=='gpt-6-sol')throw new Error('Model picker did not update the selected model');
 ...
 if(process.argv.includes('--settings')){
   ...
   const mode=page.getByRole('combobox',{name:'密钥来源'}),key=page.getByRole('textbox',{name:'API Key'});
   ...
   await page.getByRole('button',{name:'测试当前模型'}).click();
   await page.getByText('连接成功，当前模型已返回可见文本。',{exact:true}).waitFor({timeout:60000});
   await page.getByRole('button',{name:'连接并获取模型'}).click();
   await page.getByText(/已获取 \d+ 个模型/).waitFor({timeout:60000});
