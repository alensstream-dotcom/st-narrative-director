...
User-confirmed identity locks outrank guesses. Existing original character profiles fill traits not stated in the selected story; do not let a stale profile override an explicit appearance in CURRENT_TEXT. Existing original outfit presets are wardrobe references, NOT automatic current clothing; CURRENT_TEXT and time-correct previous state outrank them. If this conflict makes identity uncertain, mark uncertain and wait for user confirmation. fixed_facts fields are hair_color,hair_style,eye_color,face,build,distinctive_features, each {field,value (English),evidence (exact quote from source/profile),source (story/card/profile)}. Exclude outfit, pose, mood, location from fixed_facts. For each cast outfit, supply outfit_evidence as an exact quote in CURRENT_TEXT, time-correct PREVIOUS_CONTEXT, or character card; an outfit preset alone is never evidence of current wear. Use empty strings when clothing is unknown. Do not invent an outfit because a character has a preset. Supply outfit_class as a short lowercase general clothing tag or comma-separated tags (e.g. school uniform, coat, skirt), never a character/cosplay tag. Reuse an established class label for the same clothing type rather than inventing a synonym. outfit_specificity is 'generic' only when the source names a clothing type without fixed color, fabric, cut or accessory details; 'specified' when the source fixes such details; 'unknown' if clothing is not evidenced. The class is reusable but is NOT an instruction to copy a former outfit variant. In positive include the grounded clothing type once, and preserve any current explicit details; leave unspecified details open, without inventing them as canonical. state_updates are {evidence,characters:[{name,outfit,location,time,injury}]} with exact current evidence.
Auto: select only completed clear high-value events; can return zero. No repeated shots of the same continuing action. already_chosen contains approved frames and their source/action; a follow-through gesture, changed gaze, or lowering the same prop with the same people and place is NOT a second picture merely because the verb changed. A distinct major event may be chosen later. event_key is a stable English actor/action/object/location identity across adjacent paragraphs. A scene with only male/unknown-gender subjects is ineligible automatically; environment scene must not have a male subject disguised as scenery. Manual is unrestricted.
