...
export function validateScene(scene, raw, range, manual = false) {
  ...
  return { ...clone(scene), positive: assertEnglish(scene.positive), negative: assertEnglish(scene.negative), anchor };
}
export class AutoBudget {
  constructor(max = 2) { this.max = max; this.accepted = []; }
  accept(scene, final = false) {
...
Read ONLY up to the selected event time. PREVIOUS_CONTEXT resolves identity/location/clothes but is not another event to illustrate. No future states. Source evidence must be an exact contiguous quote in CURRENT_TEXT. At most two scenes; manual mode exactly one moment, even if selection has a sequence. Prefer its final completed meaningful action, and state the chosen moment clearly in Chinese.
...
Known original character profiles and user locks outrank guessed appearance; explicit story changes affect dynamic state only. fixed_facts fields are hair_color,hair_style,eye_color,face,build,distinctive_features, each {field,value (English),evidence (exact quote from source/profile),source (story/card/profile)}. Exclude outfit, pose, mood, location from fixed_facts. state_updates are {evidence,characters:[{name,outfit,location,time,injury}]} with exact current evidence.
