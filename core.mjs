...
IDENTITY: Respect user locks first, then relevant card/profile facts; explicit CURRENT_TEXT overrides stale traits. A name alone is not a visual identity tag. fixed_facts field names are exactly hair_color, hair_style, eye_color, face, build, distinctive_features; values must be English with exact source evidence. These are stable traits only, never clothes, pose, mood, or place. PREVIOUS_CONTEXT.outfits records the last evidenced garment actually worn in an earlier scene; keep that exact garment in later shots unless CURRENT_TEXT explicitly changes it. When CURRENT_TEXT changes clothing, use the new garment and quote that change in outfit_evidence. A wardrobe preset alone is not evidence. Unknown outfit is empty. State updates must be certain and evidence-quoted.
...
IDENTITY: Respect explicit visual locks, then matching card/profile/registry facts; current text overrides stale facts. Keep each fact with its named character. Include fixed_facts only for evidenced hair, eyes, face, build, or distinctive features. PREVIOUS_CONTEXT.outfits is the last evidenced clothing actually worn; carry the same garment into this scene unless CURRENT_TEXT explicitly changes it. Quote the current change in outfit_evidence when it occurs. A wardrobe preset alone is not evidence. Keep outfit in English and leave it empty when unknown. State updates must quote evidence and include only certain changes.
